...
void main() {
  ...
  float grain = fbm(xz * uNoiseScale * 7.2 + vec2(3.0, 31.0));
  float strata = 0.5 + 0.5 * sin(xz.y * uStrataScale * 0.85 + macro * 4.2 + detail * 1.6);
  float crackA = pow(ridge(fbm(xz * uCrackScale * 4.8 + vec2(21.0, 4.0))), 5.0);
  float crackB = pow(ridge(fbm((xz.yx + vec2(11.0, 37.0)) * uCrackScale * 4.0)), 6.0);
  float cracks = smoothstep(0.34, 0.72, max(crackA, crackB)) * (0.45 + 0.35 * grain);
  float pits = smoothstep(0.62, 0.92, fbm(xz * uNoiseScale * 14.0 + vec2(61.0, 17.0)));
  float colorMix = clamp(0.42 * macro + 0.28 * detail + 0.18 * strata + 0.08 * grain, 0.0, 1.0);
  vec3 base = mix(uColorA, uColorB, colorMix);
  base = mix(base, uColorC, smoothstep(0.52, 0.92, strata));
  base *= 0.88 + 0.18 * grain;
  base = mix(base, base * 0.5, pits * 0.26);
  base += uAccent * cracks * (0.16 + 0.28 * detail);
  base = mix(base, vec3(0.02, 0.018, 0.016), cracks * 0.14);
