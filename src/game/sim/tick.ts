...
import { survivalHordeDirector } from "./systems/survival";
...
export function advanceTick(s: GameState, intents: PlayerIntent[]): void {
  ...
  auras(s);
  wakeCamps(s);
  if (s.scenario === "survival") survivalHordeDirector(s);
  else maybeEnemyReinforcements(s);
  heroSystem(s);
  if (s.scenario !== "survival") enemyHeroSystem(s);
  movement(s);
  portals(s);
  combat(s);
  cleanupDead(s);
